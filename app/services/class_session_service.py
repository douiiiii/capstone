...
# ────────────────────────────────────────────────────────────────────
# 매칭 상태 → 세션 상태 매핑
#   매칭이 '최종확정' 이면 세션도 '완료', 그 외 활성 상태는 '예정' 으로 생성
#   '거절'/'매칭제안' 등에서는 세션을 생성하지 않음
#
#   matches.status DB CHECK 제약: {'매칭제안','수락','거절','최종확정'}
#   기존 코드의 '확정'/'완료' 를 '최종확정' 으로 통일.
# ────────────────────────────────────────────────────────────────────
...
def _initial_session_status(match_status: str) -> str:
    """매칭 상태에 따른 초기 세션 상태.
    matches.status '최종확정' → 세션도 곧바로 '완료' (이미 끝난 매칭의 백필 등).
    그 외 활성 상태('수락') → '예정'."""
    if match_status == '최종확정':
        return '완료'
    return '예정'
